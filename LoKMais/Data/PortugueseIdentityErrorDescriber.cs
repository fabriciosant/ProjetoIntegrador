...
namespace LoKMais.Data
{
    public class PortugueseIdentityErrorDescriber : IdentityErrorDescriber
    {
        ...
        public override IdentityError PasswordRequiresNonAlphanumeric() { return new IdentityError { Code = nameof(PasswordRequiresNonAlphanumeric), Description = "Sua senha deve conter pelo menos um caracter especial (ex.: @, &, *)." }; }
        public override IdentityError PasswordRequiresDigit() { return new IdentityError { Code = nameof(PasswordRequiresDigit), Description = "Sua senha deve conter ao menos um caracter numerico (Ex.: 1, 2, 3)." }; }
        public override IdentityError PasswordRequiresLower() { return new IdentityError { Code = nameof(PasswordRequiresLower), Description = "Sua senha deve conter ao menos um caracter maiúsculo (Ex.: A, B, C)" }; }
        public override IdentityError PasswordRequiresUpper() { return new IdentityError { Code = nameof(PasswordRequiresUpper), Description = "Sua senha deve conter pelo menos um caraceter minúsculo (Ex.: a, . c)" }; }
    }
}
